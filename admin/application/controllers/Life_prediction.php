...
class Life_prediction extends CI_Controller{
    ...
    function __construct () {
       ...
       public function prediction($x='',$rowno = 1)
    {
        // print_r($rowno); die;
...
// echo $this->db->last_query(); die;
        ...
        $config['base_url'] = base_url() . "Life_prediction/prediction/".$uri;
        ...
        $template['page'] = "Life_prediction/all_booking";
    ...
    private function count_total_enquiries()
    {
      ...
      elseif ($uri == "cancel_booking") 
      {
        ...
        if (isset($_GET['start_date']) || isset($_GET['end_date']) || isset($_GET['name']) || isset($_GET['email']) || isset($_GET['mobile'])) {
          ...
          if($_GET['mobile'] != '')
            {
              $this->db->or_like('mobile', $_GET['mobile']);
            }
            if($_GET['name'] != '')
            {
      ...
      elseif ($uri == "cancel_booking") 
      {
        ...
        public function refund_data()
        {
              if ($_POST) {
                ...
                $create = $this->Pdf_booking_model->get_all_result($data,'update','booking',array('id'=>$booking_id));
               if ($create) {
                ...
                $images = [];
                    if (isset($dt['confirm'])) {
                        ...
                        for ($i = 0; $i < $count; $i++) {
                            ...
                            $config['upload_path'] = 'uploads/life_prediction/';
                        ...
                        if(!empty($images)){
                            ...
                            $this->db->update('booking',['images'=>implode('|',$images)]);
