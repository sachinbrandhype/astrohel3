...
class Test_user_ctrl extends CI_Controller
{
  ...
  public function send()
  {
    ...
    $this->check_curl($message, $title, $image, $type, $link);
    // return true; 
    return redirect(base_url('Notification_send/send_to_all'));
  }

  public function check_curl($message, $title, $image, $type, $link)
  {
    ini_set('memory_limit', -1);
    $url = base_url('test_user_ctrl/curl_main_resource');
    $curl = curl_init();
    ...
    $post['type'] = $type; // our data todo in received
    $post['link'] = $link; // our data todo in received
  ...
  public function curl_main_resource()
  {
    ...
    $device_token = $this->db->query("SELECT * FROM `user` WHERE status = 1 ")->result();
