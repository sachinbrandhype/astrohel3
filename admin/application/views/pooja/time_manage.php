...
<div class="modal fade bd-example-modal-lg" id="modal-default_any">
    ...
    <form role="form" action="<?=base_url('Pooja/any_time_slots_manage');?>" method="post"  data-parsley-validate="" class="validate" enctype="multipart/form-data">
                  <div class="box-body">
                     ...
                     <div class="col-md-12">
                        <?php   
                        // print_r($pooja->stock);
                           @$monday = json_decode($pooja->stock)->mon;
                           @$tuesday = json_decode($pooja->stock)->tue;
                           @$wednesday = json_decode($pooja->stock)->wed;
                           @$thursday = json_decode($pooja->stock)->thu;
                           @$friday = json_decode($pooja->stock)->fri;
                           @$saturday = json_decode($pooja->stock)->sat;
                           @$sunday = json_decode($pooja->stock)->sun;
                             // print_r($tuesday);    
                                 ?>
                        <div class="row">
                           <div class="col-md-12">
                                          ...
                                          <div class="bootstrap-timepicker">
                                             <div class="form-group">
                                                <div class="input-group">
                                                  ...
                                                  <input type="hidden" name="any_id" value="<?php echo $pooja->id ;?>">
                                                </div>
                                             </div>
                                          </div>
                           ...
                           ?>
                            ...
                            <?php foreach ($break_time[$days[$key]] as $br_key => $breaktime) { ?>
                               <div class="form-group break_group <?php echo $value;?>">
                                 ...
                                 <div class="col-sm-3">
                                   <input type="text"  placeholder="Start Time" id="<?php echo $value.'_break_start';?>"  value="<?php echo (!empty($pooja->booking_time) && !empty($pooja->booking_time)) ? isset($breaktime['start']) ? $breaktime['start'] :'' : '';?>"  name="break[<?php echo $days[$key];?>][start][]" class="form-control timepicker start <?php echo $days[$key].'_break';?>">
                                 </div>

                                 <div class="col-sm-3">
                                   <input type="text"  placeholder="End Time" id="<?php echo $value.'_break_end';?>"  value="<?php echo (!empty($pooja->booking_time) && !empty($pooja->booking_time)) ? isset($breaktime['end']) ? $breaktime['end'] :'' : '';?>" name="break[<?php echo $days[$key];?>][end][]" class="form-control timepicker end <?php echo $days[$key].'_break';?>">
                                 </div>
                                 <div class="col-sm-2">
                                   <input type="number" min="1" placeholder="Stock" id="<?php echo $value.'_break_stock';?>"  value="<?php echo (!empty($pooja->booking_time) && !empty($pooja->booking_time)) ? isset($breaktime['stock']) ? $breaktime['stock'] :'' : '';?>" name="break[<?php echo $days[$key];?>][stock][]" class="form-control stock <?php echo $days[$key].'_break';?>">
                                 </div>
